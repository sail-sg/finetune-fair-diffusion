...
def main(args):
    ...
    gender_race_age_classifier = mobilenet_v3_large(weights=MobileNet_V3_Large_Weights.DEFAULT, width_mult=1.0, reduced_tail=False, dilated=False)
    gender_race_age_classifier._modules['classifier'][3] = nn.Linear(1280, 8, bias=True)
    
    gender_race_age_classifier.load_state_dict(torch.load(args.classifier_weight_path))
    gender_race_age_classifier.to(accelerator.device, dtype=weight_dtype)
    gender_race_age_classifier.requires_grad_(False)
    gender_race_age_classifier.eval()
    ...
    def get_face_gender_race_age(face_chips, selector=None, fill_value=-1):
        ...
        if face_chips_w_faces.shape[0] == 0:
            ...
            logits_age = torch.empty([0,2], dtype=face_chips.dtype, device=face_chips.device)
            probs_age = torch.empty([0,2], dtype=face_chips.dtype, device=face_chips.device)
            ...
            preds_age = torch.empty([0], dtype=torch.int64, device=face_chips.device)
        else:
            logits = gender_race_age_classifier(face_chips_w_faces)
        ...
        if selector != None:
            ...
            preds_age_new = torch.ones(
                [selector.shape[0]]+list(preds_age.shape[1:]), 
                dtype=preds_age.dtype, 
                device=preds_age.device
                ) * (fill_value)
            preds_age_new[selector] = preds_age
            
            probs_age_new = torch.ones(
                [selector.shape[0]]+list(probs_age.shape[1:]),
                dtype=probs_age.dtype, 
                device=probs_age.device
                ) * (fill_value)
            probs_age_new[selector] = probs_age
            
            logits_age_new = torch.ones(
                [selector.shape[0]]+list(logits_age.shape[1:]),
                dtype=logits_age.dtype, 
                device=logits_age.device
                ) * (fill_value)
            logits_age_new[selector] = logits_age
            
            return preds_gender_new, probs_gender_new, logits_gender_new, preds_race_new, probs_race_new, logits_race_new, preds_age_new, probs_age_new, logits_age_new
        else:
            return preds_gender, probs_gender, logits_gender, preds_race, probs_race, logits_race
        
    @torch.no_grad()
    def generate_dynamic_targets_gender_race_age(probs_gender, probs_race, probs_age,  w_uncertainty=False, num_samples_per_device=100):
        ...
        probs_age_2_rank = probs_age[idxs_2_rank]
        ...
        if idxs_2_rank.sum() == 0:
            ...
            if w_uncertainty:
                ...
                return targets_gender_all, uncertainty_gender_all, targets_race_all, uncertainty_race_all, targets_age_all, uncertainty_age_all
            else:
                return targets_gender_all, targets_race_all, targets_age_all
        ...
        rand_tensor_age = torch.rand([num_samples_per_device, N], dtype= probs_gender.dtype, device = probs_gender.device)

        # we hard code a perfectly balanced distribution over binary gender and 4 classes of race
        # we hard code a target age distribution
        ...
        rand_class_age = torch.zeros_like(rand_class_gender)
        rand_class_age[(rand_tensor_age>0.75)] = 1
        
        for genders, races, ages in itertools.zip_longest(rand_class_gender, rand_class_race, rand_class_age):
            freq_list = [0]*16
            for g, r, a in itertools.zip_longest(genders, races, ages):
                freq_list[g*8+r*2+a] += 1 
        ...
        for i in range(probs_gender_2_rank.shape[0]):
            ...
            prob_age = np.array(probs_age_2_rank[i].cpu())
            M_i = []
            for j in range(16):
                gender_target = gender_targets[j]
                race_target = race_targets[j]
                age_target = age_targets[j]
                
                if (age_target==[1,0]).all():
                    cost_age = math.sqrt(
                        (prob_age[0]-1)**2 + 
                        (prob_age[1]-0)**2
                        )
                elif (age_target==[0,1]).all():
                    cost_age = math.sqrt(
                        ((prob_age[0]-0)*2)**2 + 
                        (prob_age[1]-1)**2
                        )
                cost = (np.linalg.norm(prob_gender - gender_target)**2 + np.linalg.norm(prob_race - race_target)**2 + cost_age**2)**(0.5)
                M_i.append(cost)
            M.append(M_i)
        M = np.array(M)

        target_probs = np.zeros([N,16])
        ...
        target_probs_gender = torch.cat(
            [target_probs[:,:8].sum(dim=-1).unsqueeze(dim=-1), target_probs[:,8:].sum(dim=-1).unsqueeze(dim=-1)],
            dim=-1
        )
        target_probs_race = torch.cat(
            [target_probs[:,[0,1,8,9]].sum(dim=-1).unsqueeze(dim=-1), 
             target_probs[:,[2,3,10,11]].sum(dim=-1).unsqueeze(dim=-1),
             target_probs[:,[4,5,12,13]].sum(dim=-1).unsqueeze(dim=-1),
             target_probs[:,[6,7,14,15]].sum(dim=-1).unsqueeze(dim=-1)
             ],
            dim=-1
        )
        target_probs_age = torch.cat(
            [target_probs[:,[0,2,4,6,8,10,12,14]].sum(dim=-1).unsqueeze(dim=-1), 
             target_probs[:,[1,3,5,7,9,11,13,15]].sum(dim=-1).unsqueeze(dim=-1)
             ],
            dim=-1
        )
        ...
        targets_age = target_probs_age.argmax(axis=-1).to(torch.long)
        uncertainty_age = 1-target_probs_age.max(axis=-1).values
        ...
        targets_age_all = torch.ones([probs_age.shape[0]], dtype=torch.long, device=probs_age.device) * (-1)
        targets_age_all[idxs_2_rank] = targets_age
        ...
        if w_uncertainty:
            ...
            uncertainty_age_all = torch.ones([probs_age.shape[0]], dtype=probs_age.dtype, device=probs_age.device) * (-1)
            uncertainty_age_all[idxs_2_rank] = uncertainty_age
    ...
    def evaluate_process(which_text_encoder, which_unet, name, prompts, noises, current_global_step):
        ...
        for prompt_i, noises_i in itertools.zip_longest(prompts, noises):
            ...
            preds_gender_ori, probs_gender_ori, logits_gender_ori, preds_race_ori, probs_race_ori, logits_race_ori, preds_age_ori, probs_age_ori, logits_age_ori = get_face_gender_race_age(face_chips_ori, selector=face_indicators_ori, fill_value=-1)
            ...
            preds_age_ori_all = customized_all_gather(preds_age_ori, accelerator, return_tensor_other_processes=False)
            probs_age_ori_all = customized_all_gather(probs_age_ori, accelerator, return_tensor_other_processes=False)
            ...
            if accelerator.is_main_process:
                ...
                plot_in_grid_gender_race_age(
                    ...
                    preds_age=preds_age_ori_all, 
                    pred_class_probs_age=probs_age_ori_all.max(dim=-1).values,
            ...
            preds_gender, probs_gender, logits_gender, preds_race, probs_race, logits_race, preds_age, probs_age, logits_age = get_face_gender_race_age(face_chips, selector=face_indicators, fill_value=-1)
            ...
            preds_age_all = customized_all_gather(preds_age, accelerator, return_tensor_other_processes=False)
            probs_age_all = customized_all_gather(probs_age, accelerator, return_tensor_other_processes=False)
            ...
            if accelerator.is_main_process:
                ...
                plot_in_grid_gender_race_age(
                    ...
                    preds_age=preds_age_all,
                    pred_class_probs_age=probs_age_all.max(dim=-1).values,
                    )
            ...
            if accelerator.is_main_process:
                ...
                gender_gap, gender_pred_below_08, race_gap, race_pred_below_08, gender_race_gap, age_young_freq, age_old_freq, age_pred_below_08, age_gap = get_evaluate_metrics(probs_gender_all, probs_race_all, probs_age_all)
                ...
                logs_i["age_young_freq"].append( age_young_freq )
                logs_i["age_old_freq"].append( age_old_freq )
                logs_i["age_pred_below_0.8"].append( age_pred_below_08 )
                logs_i["age_gap"].append( age_gap )
    ...
    def get_evaluate_metrics(probs_gender_all, probs_race_all, probs_age_all):
        ...
        probs_age_tmp = probs_age_all[(probs_age_all!=-1).all(dim=-1)]
        preds_age_tmp = probs_age_tmp.argmax(dim=-1)
        age0_freq = (preds_age_tmp==0).float().mean().item()
        age1_freq = (preds_age_tmp==1).float().mean().item()
        age_gap = (abs(age0_freq-0.75) + abs(age1_freq-0.25))/2
        age_pred_below_08 = (probs_age_tmp.max(dim=-1).values < 0.8).float().mean().item()
        
        return gender_gap, gender_pred_below_08, race_gap, race_pred_below_08, gender_race_gap, age0_freq, age1_freq, age_pred_below_08, age_gap
    
    def apply_grad_hook_face(images, face_bboxs, face_bboxs_ori, targets_gender, preds_gender_ori, probs_gender_ori, targets_race, preds_race_ori, probs_race_ori, targets_age, preds_age_ori, probs_age_ori, factor_gender=0.2, factor_race=0.3, factor_age=0.3):
        images_new = []
        for (image, face_bbox, face_bbox_ori, 
        target_gender, pred_gender_ori, prob_gender_ori, 
        target_race, pred_race_ori, prob_race_ori, 
        target_age, pred_age_ori, prob_age_ori) in itertools.zip_longest(
            images, face_bboxs, face_bboxs_ori, 
            targets_gender, preds_gender_ori, probs_gender_ori, 
            targets_race, preds_race_ori, probs_race_ori, 
            ...
            if (face_bbox == -1).all():
                ...
                if (target_gender==pred_gender_ori) and (target_race==pred_race_ori) and (target_age==pred_age_ori):
                    grad_hook = make_grad_hook(1)
                else:
                    factors = []
                    if target_gender!=pred_gender_ori:
                        factors.append(factor_gender)
                    if target_race!=pred_race_ori:
                        factors.append(factor_race)
                    if target_age!=pred_age_ori:
                        factors.append(factor_age)
                    grad_hook = make_grad_hook(min(factors))
    ...
    def gen_dynamic_weights(
            ...
            targets_race, preds_race_ori, probs_race_ori, 
            targets_age, preds_age_ori, probs_age_ori, 
            factor_gender=0.2, factor_race=0.6, factor_age=0.6
            ):
        weights = []
        for face_indicator, target_gender, pred_gender_ori, prob_gender_ori, target_race, pred_race_ori, prob_race_ori, target_age, pred_age_ori, prob_age_ori in itertools.zip_longest(face_indicators, targets_gender, preds_gender_ori, probs_gender_ori, targets_race, preds_race_ori, probs_race_ori, targets_age, preds_age_ori, probs_age_ori):
            if (face_indicator == False).all():
                weights.append(min(factor_gender,factor_race,factor_age))
            else:
                if (target_gender==pred_gender_ori) and (target_race==pred_race_ori) and (target_age==pred_age_ori):
                    weights.append(1)
                else:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):            
            ...
            with torch.no_grad():
                ...
                targets_gender_all, uncertainty_gender_all, targets_race_all, uncertainty_race_all, targets_age_all, uncertainty_age_all = generate_dynamic_targets_gender_race_age(probs_gender_all, probs_race_all, probs_age_all, w_uncertainty=True)
                ...
                torch.distributed.broadcast(targets_age_all, src=0)
                torch.distributed.broadcast(uncertainty_age_all, src=0)
                ...
                targets_age = targets_age_all[probs_age.shape[0]*(accelerator.local_process_index):probs_age.shape[0]*(accelerator.local_process_index+1)]

                accelerator.print(f"\tNum faces w/ gender grads: {(targets_gender_all!=-1).sum().item()}/{targets_gender_all.shape[0]}, Num faces w/ race grads: {(targets_race_all!=-1).sum().item()}/{targets_race_all.shape[0]}, Num faces w/ age grads: {(targets_age_all!=-1).sum().item()}/{targets_age_all.shape[0]}")
            ...
            loss_fair_age_i = torch.ones(targets_age.shape, dtype=weight_dtype, device=accelerator.device) *(-1)
            ...
            for j in range(N_backward):
                ...
                preds_age_ori_ij = preds_age_ori[idxs_ij]
                probs_age_ori_ij = probs_age_ori[idxs_ij]
                ...
                preds_gender_ij, probs_gender_ij, logits_gender_ij, preds_race_ij, probs_race_ij, logits_race_ij, preds_age_ij, probs_age_ij, logits_age_ij = get_face_gender_race_age(face_chips_ij, selector=face_indicators_ij, fill_value=-1)
                
                images_ij = apply_grad_hook_face(
                    images_ij, face_bboxs_ij, face_bboxs_ori_ij, 
                    targets_gender_ij, preds_gender_ori_ij, probs_gender_ori_ij, 
                    targets_race_ij, preds_race_ori_ij, probs_race_ori_ij, 
                    targets_age_ij, preds_age_ori_ij, probs_age_ori_ij,
                    factor_gender=args.factor2_gender, factor_race=args.factor2_race, factor_age=args.factor2_age)
                ...
                loss_fair_age_ij = torch.ones(len(idxs_ij), dtype=weight_dtype, device=accelerator.device) *(-1)
                idxs_w_fair_age_loss = ((face_indicators_ij == True) * (targets_age_ij != -1)).nonzero().view([-1])
                loss_fair_age_ij_ = CE_loss(logits_age_ij[idxs_w_fair_age_loss], targets_age_ij[idxs_w_fair_age_loss])
                loss_fair_age_ij[idxs_w_fair_age_loss] = loss_fair_age_ij_
                ...
                idxs_w_face_feats_from_ori = (
                    ...
                    (targets_gender_ij!=-1) * (targets_gender_ij==preds_gender_ori_ij) * (probs_gender_ori_ij.max(dim=-1).values>=args.face_gender_race_age_confidence_level) *
                    (targets_race_ij!=-1) * (targets_race_ij==preds_race_ori_ij) * (probs_race_ori_ij.max(dim=-1).values>=args.face_gender_race_age_confidence_level) *
                    (targets_age_ij!=-1) * (targets_age_ij==preds_age_ori_ij) * (probs_age_ori_ij.max(dim=-1).values>=args.face_gender_race_age_confidence_level)
                ...
                dynamic_weights = gen_dynamic_weights(
                    ...
                    targets_race_ij, preds_race_ori_ij, probs_race_ori_ij,
                    targets_age_ij, preds_age_ori_ij, probs_age_ori_ij,
                    factor_gender=args.factor1_gender, factor_race=args.factor1_race, factor_age=args.factor1_age
                    )
                loss_ij = loss_fair_gender_ij + loss_fair_race_ij + loss_fair_age_ij + args.weight_loss_img * dynamic_weights * (loss_CLIP_ij + loss_DINO_ij) + args.weight_loss_face * loss_face_ij
                ...
                with torch.no_grad():
                    ...
                    loss_fair_age_i[idxs_ij] = loss_fair_age_ij.to(loss_fair_age_i.dtype)
            ...
            loss_fair_age_all = customized_all_gather(loss_fair_age_i, accelerator)
            ...
            loss_fair_age_all = loss_fair_age_all[loss_fair_age_all!=-1]
            ...
            if accelerator.is_main_process:
                ...
                logs_i["loss_fair_age"].append(loss_fair_age_all)
            ...
            # process logs
            if accelerator.is_main_process:
                for key in ["loss_fair_gender", "loss_fair_race", "loss_fair_age", "loss_face", "loss_CLIP", "loss_DINO", "loss"]:
                    if logs_i[key] == []:
                        logs_i.pop(key)
                    else:
                        logs_i[key] = torch.cat(logs_i[key])
                for key in ["gender_gap", "gender_pred_below_0.8", "race_gap", "race_pred_below_0.8", "gender_race_gap", "age_young_freq", "age_old_freq", "age_pred_below_0.8", "age_gap"]:
                    if logs_i[key] == []:
                        logs_i.pop(key)
