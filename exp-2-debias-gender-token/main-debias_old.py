...
class FaceFeatsModel(torch.nn.Module):
    ...
    def semantic_search(self, query_embeddings, selector=None, return_similarity=False):
        ...
        Args:
            ...
            genders (torch.tensor/None): if provided, will search from the same gender class
            selector (torch.tensor/None): if provided, should be a bool vector same size as the first dim of query_embeddings,
                only perform search for the selected query_embeddings
            return_similarity (True/False): whether return similarities
        
        Returns:
            target_embeddings (torch.tensor):
            similarities: if return_similarity==True
        """
...
def main(args):
    ...
    # set up face_app on all devices
    ...
    prefix_embedding = accelerator.prepare(prefix_embedding)
    ...
    def evaluation_step(current_step):
        ...
        with torch.no_grad():
            ...
            for p, p_from in itertools.zip_longest(list(prefix_embedding.module.token_embedding.parameters()), prefix_embedding_ema.shadow_params):
                p.data = p_from.data
            
        evaluate_process(text_encoder, unet, prefix_embedding, "EMA", prompts_val, noises_val, current_step)
        
        with torch.no_grad():
            for p, p_from in itertools.zip_longest(list(prefix_embedding.module.token_embedding.parameters()), list(prefix_embedding_copy.module.token_embedding.parameters())):
                p.data = p_from.data
    ...
    if args.resume_from_checkpoint:
        ...
        if not os.path.exists(args.resume_from_checkpoint):
            ...
            resume_step = resume_global_step % (args.num_update_steps_per_epoch * args.gradient_accumulation_steps)
...
# accelerate launch --config_file configs/accelerate_config.yaml debias-gender.py --config configs/debias-token.yaml